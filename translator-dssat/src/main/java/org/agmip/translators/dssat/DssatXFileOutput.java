...
//                    sbData.append(String.format(" %1$s\r\n", notes));
            ...
            for (int i = 0; i < sqArr.size(); i++) {
                ...
                copyItem(flData, rootData, "ireff");
                copyItem(flData, rootData, "bed_h");
                copyItem(flData, rootData, "bed_w");        //pmalb        
                copyItem(flData, rootData, "pmalb");//pmalb        
...
//                        sbError.append("! Warning: Incompleted record because missing data : [cul_id], and will use default value '").append(cul_id).append("'\r\n");
            ...
            for (int idx = 0; idx < flArr.size(); idx++) {
                ...
                sbData.append(String.format("%1$2s %2$-8s %3$-8s %4$5s %5$5s %6$5s %7$5s %8$5s %9$5s %10$-5s%11$5s  %12$-10s %13$s %14$s %15$5s  %16$s\r\n", // P.S. change length definition to match current way
                        ...
                        soil_id,
                        bedWidth,
                        //formatNumStr(4, secData, "bed_w", defValR),
                        //formatNumStr(5, secData, "bed_h", defValR),
                        bedHt,
